...
const Header: React.FC = () => {
  ...
  const groupedCartItems = cartItems.reduce((acc, item, index) => {
    ...
    return acc;
  }, {} as Record<string, CartAddRequest & { quantity: number; indices: number[] }>);

  const groupedItemsArray: (CartAddRequest & { quantity: number; indices: number[] })[] = Object.values(groupedCartItems);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      const orderedProducts = groupedItemsArray.map(item => productMap[item.id] || null);
        ...
        <div className={`cart-modal ${isCartOpen ? 'active' : ''}`}>
          <div className="cart-modal-content">
            <div className="cart-modal-header">
              ...
              <button onClick={toggleCart} className="cart-modal-close" aria-label="Close cart">
            ...
            {groupedItemsArray.length === 0 ? (
              ...
              <div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="cart-item-details">
                        ...
                        <p className="cart-item-price">${parseFloat(product.price)}</p>
                ...
                <Link to="/checkout" onClick={toggleCart} className="cart-checkout-button">
                  Go to Checkout
                </Link>
              </div>
            )}
